 ...
 */
mainApp.controller("AssignGroupController", ['$scope', 'groupingService', '$window', '$rootScope', 'busyIndicator', '$document',
    function ($scope, groupingService, $window, $rootScope, busyIndicator, $document) {
        ...
        var disableUsers = function () {
            ...
            $scope.filterByKeyword = function (keyword) {
                ...
                if (keyword != "") {
                    ...
                    angular.forEach($scope.contacts, function (item) {
                        if (item.display_name.toLowerCase().indexOf(key) != -1
                            || item.email_address.toLowerCase().indexOf(key) != -1) {
                            filteredContacts.push(item);
                        }
                    });
                    $scope.contacts = filteredContacts;
